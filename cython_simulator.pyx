...
cdef extern from "simulator.h":
    ...
    void scanDistance(int *dist_arr, double *sim_map_arr, double bot_x, double bot_y, double theta)
...
def cy_scanDistance(np.ndarray[int, ndim=1, mode="c"] input_dist, np.ndarray[double, ndim=2, mode="c"] input_sim_map, int x, int y, int theta):
    """
