...
cdef void measureDistance(np.ndarray[int, ndim=1, mode="c"] dist_arr, np.ndarray[double, ndim=2, mode='c'] sim_map_arr, double x, double y, double theta):
...
cdef void cy_detectHits(np.ndarray[int, ndim=1, mode="c"] dist_arr, np.ndarray[double, ndim=2, mode='c'] LPS_arr, double theta):
...
cdef void cy_updateFromLPS(double x, double y, double theta, np.ndarray[double, ndim=2, mode='c'] LPS_arr, np.ndarray[double, ndim=2, mode='c'] GPS_arr):
...
def main():
    ...
    measureDistance(distance, sim_map, x, y, theta)
    LPS[0:] = 0.5
    cy_detectHits(distance, LPS, theta)
    cy_updateFromLPS(x, y, theta, LPS, GPS)
